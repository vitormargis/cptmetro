...
function cptmetro(options) {
  ...
  spinner.start();
  return Promise.all([(0, _getMETRO2.default)(), (0, _getCPTM2.default)()]).then(function (data) {
    spinner.stop();
    ...
    if (optionLine) {
      metroData = metroData.filter(function (line) {
        return optionLine.indexOf(line.id) >= 0 ? line : null;
      });
    ...
    if (optionStatus) {
      metroData = metroData.filter(function (line) {
        return optionStatus.indexOf(statusId[line.status]) >= 0 ? line : null;
      });
      cptmData = cptmData.filter(function (line) {
        return optionStatus.indexOf(statusId[line.status]) >= 0 ? line : null;
      });
    ...
    var tableMETRO = new _cliTable2.default({ colWidths: metroData.map(function () {
        return 23;
      }) });
    var tableCPTM = new _cliTable2.default({ colWidths: cptmData.map(function () {
        return 23;
      }) });

    var metroAresponseLines = metroData.map(function (item) {
      return [_chalk2.default[item.chalk](item.name)];
    });
    var metroAresponseStatus = metroData.map(function (item) {
      return [statusWithIcon[item.status]];
    });

    var cptmAresponseLines = cptmData.map(function (item) {
      return [_chalk2.default[item.chalk](item.name)];
    });
    var cptmAresponseStatus = cptmData.map(function (item) {
      return [statusWithIcon[item.status]];
    });

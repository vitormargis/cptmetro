...
function cptmetro(options) {
  ...
  return Promise.all([(0, _getMETRO2.default)(), (0, _getCPTM2.default)()]).then(function (data) {
    ...
    var aAresponseStations = metroData.map(function (item) {
      return ['Stations: ' + item.info.stations];
    });
    var aAresponseLength = metroData.map(function (item) {
      return ['Length: ' + item.info.length];
    });
    var aAresponseInalguration = metroData.map(function (item) {
      return ['Inalguration: ' + item.info.inalguration];
    });
    ...
    var bAresponseStations = metroData.map(function (item) {
      return ['Stations: ' + item.info.stations];
    });
    var bAresponseLength = metroData.map(function (item) {
      return ['Length: ' + item.info.length];
    });
    var bAresponseInalguration = metroData.map(function (item) {
      return ['Inalguration: ' + item.info.inalguration];
    });

    tableMETRO.push(aAresponseLines, aAresponseStatus, aAresponseStations, aAresponseLength, aAresponseInalguration);

    tableCPTM.push(bAresponseLines, bAresponseStatus, bAresponseStations, bAresponseLength, bAresponseInalguration);
